...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < threads.size(); i++){
        ...
        double lin_time = 0;
        clock.Start();
        nnzCPerCol = symbolicSpAddRegularStatic<uint32_t,uint32_t,float,uint32_t>(vec[0], vec[1]);
        OutPairwiseLinear = SpAddRegularStatic<uint32_t,uint32_t,float,uint32_t>(vec[0], vec[1], nnzCPerCol);
        clock.Stop();
        lin_time += clock.Seconds();
        for (int j = 2; j < k; j++){
            clock.Start();
            nnzCPerCol = symbolicSpAddRegularStatic<uint32_t,uint32_t,float,uint32_t>(&OutPairwiseLinear, vec[j]);
            OutPairwiseLinear = SpAddRegularStatic<uint32_t,uint32_t,float,uint32_t>(&OutPairwiseLinear, vec[j], nnzCPerCol);
            clock.Stop();
            lin_time += clock.Seconds();
        }
        if(type == 0){
